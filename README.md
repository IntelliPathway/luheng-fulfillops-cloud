...
当前版本：`v0.9.0`。本版本新增支付回执 HMAC 验签、租户/Provider 事件幂等、案件回款匹配，以及不可变回款与佣金账簿。钱指标由整数分账簿汇总，不再由浏览器演示状态或浮点快照裁决；退款、结算和实收均受前序事件余额约束。
...
本项目保留 Vite 与 Sites 兼容构建。`v0.9.0` 的 CI 基线为 64 项后端、15 项前端领域/API 契约和 4 项站点构建测试，共 83 项；本地无 PostgreSQL 服务时为 63 项后端通过、1 项专项跳过。新增覆盖坏签名/过期签名零落库、回执幂等冲突、跨租户匹配与同号事件隔离、退款原比例与金额上限、计佣边界、结算/实收余额约束、密钥密文保存，以及 v0.4 到当前版本的 PostgreSQL 原地升级。后续仍需在正式云账号、企业 IdP、DeepSeek 凭证与真实支付 Provider 下完成环境验收。
