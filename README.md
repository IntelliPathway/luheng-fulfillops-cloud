...
当前版本：`v0.9.1`。在 v0.9 资金证据链基础上完成 Sites 托管适配：线上测试版明确区分“在线交互演示”和“业务 API 已连接”，未接入 FastAPI 时 `/api/*` 失败关闭并返回结构化 503；静态资源与 SPA 路由由 Sites Worker 提供，并统一附加浏览器安全响应头。
...
本项目保留 Vite 与 Sites 兼容构建。`v0.9.1` 的 CI 基线为 64 项后端、15 项前端领域/API 契约和 5 项站点构建/安全测试，共 84 项；本地无 PostgreSQL 服务时为 63 项后端通过、1 项专项跳过。新增覆盖坏签名/过期签名零落库、回执幂等冲突、跨租户匹配与同号事件隔离、退款原比例与金额上限、计佣边界、结算/实收余额约束、密钥密文保存、Sites API 失败关闭和安全响应头，以及 v0.4 到当前版本的 PostgreSQL 原地升级。后续仍需在正式云账号、企业 IdP、DeepSeek 凭证与真实支付 Provider 下完成环境验收。
