...
`v1.1.0` 后端基线提供数据库成员权限、企业 OIDC/JWKS、持久异步作业、独立 Worker、受控 Agent/模型 Gateway、策略 Maker–Checker、资产导入与权威目录、电话和支付验签事件、不可变财务账簿、保护事件、分期履约、请求观测和试点评分卡。生产启动配置、迁移、演示种子和首租户初始化已经分离；Sites 只承载前端交互沙箱。
