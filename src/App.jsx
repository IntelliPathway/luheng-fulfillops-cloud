...
import {activityApi,agentApi,authApi,integrationApi,jobApi,normalizeIntegrationOverview} from './api';
...
const defaultQueueHealth={mode:'inline',status:'inline',active_workers:0,queued_jobs:0,running_jobs:0,stale_jobs:0,latest_heartbeat_at:null,lease_seconds:60};
export function App(){
 ...
 useEffect(()=>{let active=true;setBackendStatus('checking');setQueueHealth(defaultQueueHealth);Promise.all([integrationApi.get(tenant),authApi.session(tenant),agentApi.gateway(tenant),jobApi.queueHealth(tenant).catch(()=>defaultQueueHealth)]).then(([payload,session,gateway,queue])=>{if(!active)return;const normalized=normalizeIntegrationOverview(payload);setServiceConfigsByTenant(all=>({...all,[tenant]:{...all[tenant],...normalized.services}}));setIntegrationStatesByTenant(all=>({...all,[tenant]:normalized.state}));setIdentity(session);setAgentGateway(gateway);setQueueHealth(queue);setBackendStatus('connected')}).catch(()=>{if(active){setBackendStatus('offline');setQueueHealth(defaultQueueHealth);setAgentGateway(current=>({...current,connected:false,mode:'local-demo'}))}});return()=>{active=false}},[tenant]);
