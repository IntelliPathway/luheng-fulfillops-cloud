...
import {activityApi,agentApi,authApi,integrationApi,normalizeIntegrationOverview} from './api';
...
export function App(){
 ...
 useEffect(()=>{let active=true;setBackendStatus('checking');Promise.all([integrationApi.get(tenant),authApi.session(tenant),agentApi.gateway(tenant)]).then(([payload,session,gateway])=>{if(!active)return;const normalized=normalizeIntegrationOverview(payload);setServiceConfigsByTenant(all=>({...all,[tenant]:{...all[tenant],...normalized.services}}));setIntegrationStatesByTenant(all=>({...all,[tenant]:normalized.state}));setIdentity(session);setAgentGateway(gateway);setBackendStatus('connected')}).catch(()=>{if(active){setBackendStatus('offline');setAgentGateway(current=>({...current,connected:false,mode:'local-demo'}))}});return()=>{active=false}},[tenant]);
