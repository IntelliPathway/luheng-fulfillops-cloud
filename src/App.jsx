...
const defaultAuthHealth={mode:'unconfigured',status:'degraded',issuer_configured:false,audience_configured:false,jwks_configured:false,allowed_algorithms:[],required_claims:['sub','exp','iat'],tenant_claim:null,leeway_seconds:0,jwks_cache_seconds:300,dev_header_enabled:false,dev_token_enabled:false,detail:'认证策略仅管理员可见'};
export function App(){
 ...
 useEffect(()=>{let active=true;setBackendStatus('checking');setQueueHealth(defaultQueueHealth);setSecretHealth(defaultSecretHealth);setAuthHealth(defaultAuthHealth);setLatestReplay(null);Promise.all([integrationApi.get(tenant),authApi.session(tenant),agentApi.gateway(tenant),jobApi.queueHealth(tenant).catch(()=>defaultQueueHealth),securityApi.secretHealth(tenant).catch(()=>defaultSecretHealth),securityApi.authHealth(tenant).catch(()=>null),agentApi.replays(tenant).catch(()=>[])]).then(([payload,session,gateway,queue,secrets,auth,replays])=>{if(!active)return;const normalized=normalizeIntegrationOverview(payload);setServiceConfigsByTenant(all=>({...all,[tenant]:{...all[tenant],...normalized.services}}));setIntegrationStatesByTenant(all=>({...all,[tenant]:normalized.state}));setIdentity(session);setAgentGateway(gateway);setQueueHealth(queue);setSecretHealth(secrets);setAuthHealth(auth||{...defaultAuthHealth,mode:session.auth_mode==='oidc'?'oidc':'unconfigured'});setLatestReplay(replays[0]||null);setBackendStatus('connected')}).catch(()=>{if(active){setBackendStatus('offline');setQueueHealth(defaultQueueHealth);setSecretHealth(defaultSecretHealth);setAuthHealth(defaultAuthHealth);setLatestReplay(null);setAgentGateway(current=>({...current,connected:false,mode:'local-demo'}))}});return()=>{active=false}},[tenant]);
 ...
 const runModelReplay=async()=>{if(backendStatus!=='connected')return null;const job=await agentApi.replay(tenant,setActiveJob);const replay=await agentApi.replayRun(tenant,job.result.replay_run_id);setLatestReplay(replay);return replay};
