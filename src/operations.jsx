...
export function Strategy(){
  ...
  const [harnesses,setHarnesses]=useState({adapters:[],results:[],recommendation:null});
  ...
  useEffect(()=>{if(a.backendStatus!=='connected')return;let active=true;Promise.all([harnessApi.catalog(a.tenant),harnessApi.leaderboard(a.tenant)]).then(([catalog,board])=>{if(active)setHarnesses({adapters:catalog.adapters,results:board.results,recommendation:board.recommendation})}).catch(error=>a.notify(`Harness 评测读取失败：${error.message}`));return()=>{active=false}},[a.backendStatus,a.tenant,a.latestReplay]);
  useEffect(()=>{if(a.backendStatus!=='connected')return;let active=true;strategyExperimentApi.list(a.tenant).then(rows=>active&&setExperiments(rows)).catch(error=>a.notify(`策略实验读取失败：${error.message}`));return()=>{active=false}},[a.backendStatus,a.tenant]);
  ...
  return <><PageHead title="策略与知识" description="授权政策控制能做什么；知识库帮助 Agent 正确解释。"><Button onClick={()=>a.setCopilotOpen(true)}>向知识库提问</Button></PageHead><Tabs value={tab} onChange={setTab} items={['授权策略','策略实验','知识库','评估记录']}/>
    ...
    {tab==='策略实验'&&<><div className="notice"><Flask size={19}/>候选策略最多承接 50% 案件；SHA-256 稳定分流，创建者不能启动自己的实验。</div><div className="experiment-grid">{experiments.map(row=><article key={row.id}><div><span className="eyebrow">{row.id} · {row.package_id}</span><h3>{row.name}</h3><p>{row.hypothesis}</p></div><div className="experiment-metrics"><span><b>{row.allocation_bps/100}%</b><small>候选流量</small></span><span><b>v{row.control_policy_version} → v{row.candidate_policy_version}</b><small>策略版本</small></span></div><Badge status={row.status==='running'?'running':row.status==='stopped'?'completed':'paused'}>{row.status==='running'?'运行中':row.status==='stopped'?'已停止':'待复核'}</Badge></article>)}{!experiments.length&&<Empty title="尚无策略实验" description="通过受治理 API 创建实验草稿，由另一名管理员核验后启动。"/>}</div></>}
    ...
    {tab==='评估记录'&&<><section className="harness-recommendation"><Flask size={24}/><div><span className="eyebrow">HARNESS SELECTOR</span><h3>{harnesses.recommendation?`推荐 ${harnesses.recommendation.provider} · ${harnesses.recommendation.profile}`:'等待至少一次完成的受控回放'}</h3><p>质量 70% · 可靠性 20% · 成本效率 10%；生产启用仍要求至少 3 次回放和管理员确认。</p></div></section><div className="adapter-grid">{harnesses.adapters.map(row=><article key={row.id}><div><Brain size={20}/><b>{row.name}</b></div><p>{row.features.join(' · ')}</p><Badge status={row.status==='available'?'completed':row.status==='compatible'?'paused':'neutral'}>{row.status}</Badge></article>)}</div><div className="evaluation-history">{harnesses.results.map(row=><article key={`${row.provider}-${row.profile}`}><Flask size={22}/><span><b>{row.provider} · {row.profile}</b><small>{row.run_count} 次回放 · 质量 {row.quality_percent}% · 可靠性 {row.reliability_percent}% · 均次 ${row.average_cost_usd}</small></span><Badge status={row.run_count>=3?'completed':'paused'}>{row.score} 分</Badge></article>)}{!harnesses.results.length&&<Empty title="尚无可比较回放" description="在 AI 与渠道中配置模型后，从 Agent 页面运行同一评测集。"/>}</div></>}
  </>;
}
